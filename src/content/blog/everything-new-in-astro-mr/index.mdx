...
Astro MR supports the base path, so you can host your site in the nested "subdirectory". Just set the `base` property in the `astro.config.mjs` file.
...
Tags are available in Astro MR. Yes, the feature has been ported from Astro Micro, but in Astro MR
...
Astro MR allows to set up a graphical logo. The max dimensions are set to 240px width and 50px height. Your image can be larger, but it will be rendered to fit the dimensions.
...
Astro MR has the "dev" in the name not only because it's tailored for the software engineers' needs.
...
It's more maintainable and easier to extend now, so if you want to add new features or change the existing ones, you should find it easier to do in Astro MR than in the original Astro Micro / Astro Nano.

## Original Astro Micro changelog

Astro MR is based on [Astro Micro](https://astro-micro.vercel.app/) theme ([commit a234f5e](https://github.com/trevortylerlee/astro-micro/tree/a234f5e520b258f5ede923f8b8a4cbba03619158) to be precise).

Below is the original changelog of Astro Micro describing differences between Astro Nano that remained in Astro MR.

See [Everything new in the Astro MR](../everything-new-in-astro-milidev) to see what's new in Astro MR.
